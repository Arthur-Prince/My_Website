...
const float dt = 1.0;
const float border_h = 5.0;
const float mass = 0.5;
const float fluid_rho = 0.5;
...
bool isNearBorder(vec2 pos, vec2 resolution) {
	float margin = 10.;

	return (pos.x <= margin) || (pos.x >= resolution.x - margin) 
	|| (pos.y <= margin) || (pos.y >= resolution.y - margin);
